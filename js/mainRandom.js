...
function range(){
	...
	range.left.x = [ _photoModel.w/3, _photoModel.w ];
	range.left.y = [ _photoModel.h/3, _wrapModel.h-_photoModel.h/3 ];
	range.right.x = [ _wrapModel.w-_photoModel.w, _wrapModel.w-_photoModel.w/3 ];
...
function rsort(n){
	...
	for ( s in photos_left ) {
		...
		photoOfLeft.style.left = randomPhoto( _range.left.x ) + 'px';
		photoOfLeft.style.top  = randomPhoto( _range.left.y ) + 'px';
	...
	for ( s in photos_right ) {
		...
		photoOfRight.style.left = randomPhoto( _range.right.x ) + 'px';
		photoOfRight.style.top  = randomPhoto( _range.right.y ) + 'px';
